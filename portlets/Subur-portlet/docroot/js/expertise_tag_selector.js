AUI.add(
	'expertise-tags-selector',
	function(A) {
		var Lang = A.Lang;

		var AArray = A.Array;

		var NAME = 'expertise-tags-selector';

		var CSS_INPUT_NODE = 'lfr-expertisetag-selector-input';

		var CSS_NO_MATCHES = 'no-matches';

		var CSS_POPUP = 'lfr-expertisetag-selector-popup';

		var CSS_TAGS_LIST = 'lfr-expertisetag-selector-list';
		...
		var TPL_TAG = new A.Template(
			'<fieldset class="{[(!values.tags || !values.tags.length) ? "', CSS_NO_MATCHES, '" : "', STR_BLANK, '" ]}">',
				...
				'</tpl>',
				'<div class="lfr-expertisetag-message">{message}</div>',
			'</fieldset>'
		);
		...
		var ExpertiseTagsSelector = A.Component.create(
			{
				ATTRS: {
					allowAnyEntry: {
						value: true
				...
				prototype: {
					renderUI: function() {
						...
						ExpertiseTagsSelector.superclass.renderUI.apply(instance, arguments);
					...
					bindUI: function() {
						...
						ExpertiseTagsSelector.superclass.bindUI.apply(instance, arguments);
					...
					syncUI: function() {
						...
						ExpertiseTagsSelector.superclass.syncUI.apply(instance, arguments);
		...
		Liferay.ExpertiseTagsSelector = ExpertiseTagsSelector;
	},
	'',
	{
