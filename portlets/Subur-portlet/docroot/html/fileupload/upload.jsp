...
long repositoryId =0;// BeanParamUtil.getLong(fileEntry, request, "repositoryId");
...
long folderId = SuburFolderUtil.getFolderId(pRequest, themeDisplay);// 0;//BeanParamUtil.getLong(fileEntry, request, "folderId");*/
%>
		...
		<aui:row>
			<aui:col width="<%= 50 %>">
				...
				<%
				Date expirationDate = new Date(System.currentTimeMillis() + 20  * Time.MINUTE);
			...
			<aui:col width="<%= 50 %>">
				<div class="common-file-metadata-container hide selected" id="<portlet:namespace />commonFileMetadataContainer">
					<liferay-util:include page="/html/fileupload/multiple_upload_resources.jsp" servletContext="<%=this.getServletContext() %>" />
					<%@ include file="/html/fileupload/multiple_upload_resources.jsp" %>
				</div>
