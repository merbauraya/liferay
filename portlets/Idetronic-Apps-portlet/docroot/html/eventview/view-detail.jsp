...
<%
	...
	CalendarBooking calendarBooking =  CalendarBookingLocalServiceUtil.fetchCalendarBooking(eventId);
...
//CalendarBooking calendarBooking = event;//(CalendarBooking)request.getAttribute(WebKeys.CALENDAR_BOOKING);

Calendar calendar = calendarBooking.getCalendar();

long startTime =calendarBooking.getStartTime();//  BeanParamUtil.getLong(calendarBooking, request, "startTime");

java.util.Calendar startTimeJCalendar = JCalendarUtil.getJCalendar(startTime, userTimeZone);

long endTime = calendarBooking.getEndTime();// BeanParamUtil.getLong(calendarBooking, request, "endTime");
...
<aui:form action="<%= invokeTransitionURL %>" method="post" name="fm">
	...
	<aui:fieldset>
		<aui:button-row>
			...
			<%
			boolean hasManageBookingsPermission = CalendarPermission.contains(permissionChecker, calendar, com.idetronic.eventview.util.ActionKeys.MANAGE_BOOKINGS);
			%>
