...
public class EprintFinderImpl extends BasePersistenceImpl<Eprint>
	...
	public static final String SQL_FINDBY_DIVISIONID =
            EprintFinderImpl.class.getName() + ".findByDivisionId";
	
	private static final Log log = LogFactoryUtil.getLog(EprintFinderImpl.class);
	...
	{
		Session session = null; 
		log.info("findByDivision");
		try
		{
			session = openSession();
			String sql = CustomSQLUtil.get(SQL_FINDBY_DIVISIONID);
			SQLQuery q = session.createSQLQuery(sql);
	        q.setCacheable(false);
	        q.addEntity("Eprint", EprintImpl.class);
	        QueryPos qPos = QueryPos.getInstance(q);
	        qPos.add(subjectId);
	        log.info(qPos.toString());
