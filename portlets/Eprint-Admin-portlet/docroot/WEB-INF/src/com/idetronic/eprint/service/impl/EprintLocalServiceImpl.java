...
public class EprintLocalServiceImpl extends EprintLocalServiceBaseImpl {
			                ...
			                Date createdDate,Date modifiedDate,ServiceContext serviceContext)
	{
		...
		try {
			...
			eprint = eprintPersistence.update(eprint);
	...
	public Hits search(
			long groupId, long userId, long creatorUserId, String keyword,
			int start, int end)
		...
		searchContext.setAttribute(Field.KEYWORD_SEARCH, keyword);
		searchContext.setAttribute(Field.TITLE, keyword);
		searchContext.setAttribute(Field.DESCRIPTION,keyword);
	...
	public Hits getHits(String keyword, long companyId, long groupId) {
		...
		String[] terms = {Field.DESCRIPTION,Field.TITLE,Field.KEYWORD_SEARCH};
		try {
	...
	public List<Eprint> findByDivisionId(String subjectId,int begin,int end) 
	
	{
		return EprintFinderUtil.findByDivisionId(subjectId, begin, end);
	}
